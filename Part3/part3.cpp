...
class IStudentScoreCalculator {
public:
    virtual ~IStudentScoreCalculator() = default;
    virtual void calculateOverallScore() = 0;
};

class StudentScoreAdapter : public IStudentScoreCalculator {
private:
    unique_ptr<Student> student;

public:
    StudentScoreAdapter(unique_ptr<Student> s) : student(move(s)) {}

    void calculateOverallScore() override {
    ...
    friend ostream& operator<<(ostream& os, const StudentScoreAdapter& adapter) {
        os << *adapter.student;
        return os;
    }
};
...
public:
    static unique_ptr<Student> createStudent(const string& type, const string& name, int score, const string& schoolName, const string& dob, const string& courseName) {
        if (type == "uni") {
            return make_unique<UniStudent>(name, score, schoolName, dob, courseName);
        } else if (type == "college") {
            return make_unique<CollegeStudent>(name, score, schoolName, dob, courseName);
        } else {
...
int main() {
    ...
    auto steve = StudentFactory::createStudent("uni", "Steve", 100, "MIT", "1995-12-12", "Computer Science");
    auto john = StudentFactory::createStudent("college", "John", 100, "Harvard", "1991-12-12", "Computer Engineering");

    vector<unique_ptr<Student>> students;
    students.push_back(move(steve));
    students.push_back(move(john));
